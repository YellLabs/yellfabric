...
@runs_once
def fetch_from_repo():
    # repo_base should be something like http://cis-dev.local:8080/artifactory/libs-release-local/com/yelllabs
    prompt("Base URL for this project:", "repo_base")
    prompt("Version:", "proj_version")
    fetch = {}
    fetch[env.war_file] = "%(repo_base)s/%(project_name)s/%(proj_version)s/%(project_name)s-%(proj_version)s.war" % env
    fetch[env.app_config_archive] = "%(repo_base)s/%(project_name)s/%(proj_version)s/%(project_name)s-%(proj_version)s-config.tar.gz" % env
    fetch[env.sql_archive] = "%(repo_base)s/%(project_name)s/%(proj_version)s/%(project_name)s-%(proj_version)s-sql.tar.gz" % env
